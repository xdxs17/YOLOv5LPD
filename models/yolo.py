...
def parse_model(d, ch):  # model_dict, input_channels
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, Focus, SPP, C3, BiFPN]:
            c1, c2 = ch[f] if isinstance(f, int) else sum(ch[x] for x in f), args[0]
            c2 = make_divisible(c2 * gw, 8)
            args = [c1, c2, *args[1:]]
        elif m is nn.Upsample:
            args = args  # nn.Upsample 不需要通道参数
        elif m is Concat:
            c2 = args[1] if len(args) > 1 else sum(ch[x] for x in f)  # 使用 YAML 的 c2 或计算总和
            args = [1, c2]  # dimension=1, c2
        else:
            c2 = ch[f]
            args = [ch[f], *args]
